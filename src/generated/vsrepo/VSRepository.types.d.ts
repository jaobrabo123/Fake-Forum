...
type IsArrayFilter<S extends string> = S extends `${string}${"NotIn" | "In"}`
    ? true
    : false;

type ParseRelation<S extends string> =
    S extends `${infer Rel}Without${infer Rest}`
        ? [Uncapitalize<Rel>, "isNot", Rest]
        : S extends `${infer Rel}With${infer Rest}`
          ? [Uncapitalize<Rel>, "is", Rest]
          : S extends `${infer Rel}Some${infer Rest}`
            ? [Uncapitalize<Rel>, "some", Rest]
            : S extends `${infer Rel}Every${infer Rest}`
              ? [Uncapitalize<Rel>, "every", Rest]
              : S extends `${infer Rel}None${infer Rest}`
                ? [Uncapitalize<Rel>, "none", Rest]
                : null;
            ...
            ? IsArrayFilter<S> extends true
                ...
                : NonNullable<T[FieldName]> extends object | any[]
                  ? I extends { whereInput: infer W }
                      ...
                      : T[FieldName]
                  : T[FieldName]
            : ParseRelation<S> extends [
                    infer Rel extends keyof T,
                    any,
                    infer Rest extends string,
                ]
              ? GetFieldType<
                    NonNullable<T[Rel]> extends any[]
                        ? NonNullable<T[Rel]>[number]
                        : NonNullable<T[Rel]>,
                    Rest,
                    unknown
                >
...
type IsNoArgModifier<S extends string> =
    S extends `${string}${"StartsWith" | "EndsWith"}`
        ? false
        : S extends `${string}${NoArgModifiers | "With"}`
          ? true
          : false;

type IsOptional<S extends string> = S extends `${string}Optional`
    ? true
    : false;
type StripOptional<S extends string> = S extends `${infer Base}Optional`
    ? Base
    : S;

type MapToContractTypes<T, Arr extends string[], I> = Arr extends [
        ...
        : StripOptional<First> extends infer CleanFirst extends string
          ? IsNoArgModifier<CleanFirst> extends true
              ...
              : IsOptional<First> extends true
                ? [
                      GetFieldType<T, CleanFirst, I> | undefined,
                      ...MapToContractTypes<T, Rest, I>,
                  ]
                : [
                      GetFieldType<T, CleanFirst, I>,
                      ...MapToContractTypes<T, Rest, I>,
                  ]
          : never
    : [];

type HasMultipleANDs<S extends string> =
    S extends `${string}AND${string}AND${string}` ? true : false;

type SplitAllTokens<S extends string> = S extends `${infer L}AND${infer R}`
    ? [...SplitAllTokens<L>, ...SplitAllTokens<R>]
    : S extends `${infer L}And${infer R}`
      ? [...SplitAllTokens<L>, ...SplitAllTokens<R>]
      : S extends `${infer L}Or${infer R}`
        ? [...SplitAllTokens<L>, ...SplitAllTokens<R>]
        : S extends ""
          ? []
          : [S];
...
type FilterNonEmpty<T extends unknown[]> = T extends [
    infer First,
    ...infer Rest,
]
    ? First extends ""
...
type ResolveReturnType<M extends string, TSelected> = M extends "existsBy"
    ? boolean
            ...
            : M extends "createMany" | "createManyAndReturn"
              ...
              : M extends "updateManyBy" | "updateManyAndReturnBy"
                ...
                : M extends "findWhere" | "findListWhere"
                  ? [
                        where: I extends { whereInput: infer W }
                            ? NonNullable<W>
                            : unknown,
                    ]
                  : []),
    ...(R extends `${string}PaginatedAndOrdered`
        ? [
              pagination: PaginationOptions<
                  I extends { cursorInput: infer C } ? C : unknown
              >,
        ...
        : R extends `${string}OrderedAndPaginated`
          ...
          : R extends `${string}Paginated`
            ...
            : R extends `${string}Ordered`
              ? [
                    order: I extends { orderByInput: infer OB }
                        ? OB
                        : OrderOptions,
                ]
              : []),
...
type PrismaDelegate<M extends Prisma.ModelName> =
    Uncapitalize<M> extends keyof DbClient ? DbClient[Uncapitalize<M>] : never;

type FullModelType<M extends Prisma.ModelName> =
    Prisma.Result<PrismaDelegate<M>, {}, "findMany"> extends Array<infer U>
        ? U
        : never;

type SelectedModel<M extends Prisma.ModelName, S, SelectModels> = [S] extends [
    false,
]
    ? FullModelType<M>
    : [S] extends [never]
...
type CleanFields<R extends string> = R extends `${infer F}PaginatedAndOrdered`
    ? F
...
type MethodFn<
    ...
    R extends string,
    SelectModels,
    DefaultSelect extends keyof SelectModels | false,
    I,
> = <S extends keyof SelectModels | false = DefaultSelect>(
    ...args: [
        ...ExtractFields<T, CleanFields<R>, I>,
        ...ExtraArgs<MethodName, R, I>,
        options?: MethodOptions<S>,
    ]
) => Promise<ResolveReturnType<MethodName, SelectedModel<M, S, SelectModels>>>;

type GetMappedMethod<K extends string, MethodConf> = K extends `findBy${string}`
    ? MethodConf extends { fbMode: "one" }
        ? "findByOne"
        : "findByList"
    : K extends `existsBy${string}`
      ? "existsBy"
      : K extends `findUniqueBy${string}`
        ? "findUnique"
        : K extends `findFirstBy${string}` | `findFirst${string}`
          ? "findFirst"
          : K extends `findManyBy${string}` | `findMany${string}`
            ? "findMany"
            : K extends `createManyAndReturn${string}`
              ? "createManyAndReturn"
              : K extends `createMany${string}`
                ? "createMany"
                : K extends `create${string}`
                  ? "create"
                  : K extends `updateManyAndReturnBy${string}`
                    ? "updateManyAndReturnBy"
                    : K extends `updateManyBy${string}`
                      ? "updateManyBy"
                      : K extends `updateBy${string}`
                        ? "updateBy"
                        : K extends `upsertBy${string}`
                          ? "upsertBy"
                          : K extends `deleteManyBy${string}`
                            ? "deleteManyBy"
                            : K extends `deleteBy${string}`
                              ? "deleteBy"
                              : K extends `countBy${string}` | `count${string}`
                                ? "count"
                                : K extends `findListWhere${string}`
                                  ? "findListWhere"
                                  : K extends `findWhere${string}`
                                    ? "findWhere"
                                    : never;

type ExtractPatternBase<K extends string> = K extends `${string}By${infer R}`
    ? R
    : K extends
            | `findFirst${infer R}`
            | `findMany${infer R}`
            | `createManyAndReturn${infer R}`
            | `createMany${infer R}`
            | `create${infer R}`
            | `count${infer R}`
            | `findWhere${infer R}`
            | `findListWhere${infer R}`
      ? R
      : "";
...
type MethodFactory<
    ...
    MethodConf,
> = MethodFn<
    GetMappedMethod<K, MethodConf>,
    T,
    M,
    ExtractPatternBase<K>,
    SelectModels,
    DefaultSelect,
    I
>;
...
type ResolveSelectModel<MethodConf, GlobalConf, SelectModels> =
    MethodConf extends { selectModel: infer S }
        ? S extends false
            ? false
            : S extends keyof SelectModels
              ? S
...
type ExtractPkName<T, Config> = Config extends { pkName: infer PK }
    ? PK extends keyof T
        ...
        : never
    : never;
type ExtractSelectModels<Config> = Config extends { selectModels: infer SM }
    ? SM
    : {};
type ExtractDefaultSelect<Config> = Config extends {
    defaultSelectModel: infer D;
}
    ? D
    : never;
...
type DynamicMethods<T, M extends Prisma.ModelName, Config, I> = Config extends {
    methods: infer Methods;
}
    ? {
          [K in keyof Methods as Methods[K] extends { map: true }
              ? K
 ...
 */
export type SelectModel<M extends Prisma.ModelName> =
    PrismaModelInputs<M>["select"];

/**
 * Mapa de selects nomeados e reutilizáveis para um modelo Prisma.
 */
export type SelectModels<M extends Prisma.ModelName> = Record<
    string,
    SelectModel<M>
>;
 ...
 */
export type WhereModel<M extends Prisma.ModelName> =
    PrismaModelInputs<M>["whereInput"];

/**
 * Tipo do objeto `orderBy` de um modelo Prisma.
 */
export type OrdenationModel<M extends Prisma.ModelName> =
    PrismaModelInputs<M>["orderByInput"];

/**
 * Opções de paginação com cursor tipado para um modelo Prisma.
 */
export type PaginationModel<M extends Prisma.ModelName> = PaginationOptions<
    PrismaModelInputs<M>["cursorInput"]
>;
...
> = C extends { baseMethods: { get: { active: false } } }
    ...
    : {
          get<
              S extends keyof TSelects | false = ResolveMethodDefaultSelect<
                  Config,
                  C,
                  "get",
                  TSelects
...
export type DistributiveOmit<T, K extends keyof any> = T extends any
    ? Omit<T, K>
    : never;
type ExtractUnionProp<T, K extends PropertyKey> = T extends any
    ? K extends keyof T
        ? T[K]
        : never
    : never;
...
> =
    NonNullable<
        ExtractUnionProp<PrismaModelInputs<M>["createInput"], K>
    > extends { create?: infer C }
        ? Exclude<C, any[]>
        : never;
...
> =
    NonNullable<TField> extends any[]
        ...
        : NonNullable<TField> extends object
          ?
                | ExtractNestedCreateInput<M, K>
                | (TRelationConfig extends
                      | { mode: "oto"; restriction: "set" }
                      | { mode: "mto"; nullAble: true }
                      ? null
                      : never)
          : never;
 ...
 */
export type UpsertWithRelations<
    T,
    M extends Prisma.ModelName,
    TRelations,
> = DistributiveOmit<ModelUpsertInput<M>, keyof TRelations> & {
    [K in Extract<keyof TRelations, keyof T>]?: RelationPayload<
        T[K],
        TRelations[K],
        M,
        K
...
> = C extends { baseMethods: { save: { active: false } } }
    ...
    : {
          ...
          >(
              ...
              RefineSaveResult<
                  ResolveCurrentReturn<M, TSelects, S, TDefault>,
                  O
              >
          >;
      };
 ...
 */
export type ExtractRelationConfig<TField> =
    NonNullable<TField> extends infer NonNull
        ? NonNull extends Date | Buffer | Uint8Array
            ? never
            : NonNull extends any[]
              ? NonNull[number] extends object
                  ? {
                        pk: keyof NonNull[number];
                        mode: "otm" | "mtm";
                        restriction: "set" | "add";
                    }
                  : never
              : NonNull extends object
                ?
                      | {
                            pk: keyof NonNull;
                            mode: "oto";
...
export type RepositoryRelations<T> = {
    [K in keyof T as ExtractRelationConfig<T[K]> extends never
        ? never
        : K]?: ExtractRelationConfig<T[K]>;
};
...
export type BuiltRepository<
    T extends object,
    M extends Prisma.ModelName,
    Config extends RepoConfig<T, M, any>,
    C extends BuildConfig<any> | undefined,
> = {
    /**
     ...
     */
    extend<E>(
        extensionFunc: (repo: BuiltRepository<T, M, Config, C>) => E,
    ): BuiltRepository<T, M, Config, C> & E;
} & DynamicMethods<T, M, Config, PrismaModelInputs<M>> &
...
export declare class VSRepository<
    T extends object,
    M extends Prisma.ModelName,
    const Config extends RepoConfig<T, M, any> = RepoConfig<T, M, any>,
> {
     ...
     */
    build<C extends BuildConfig<any>>(
        prisma: DbClient,
        config?: C,
...
export type RepositoryOf<
    TRepo,
    C extends BuildConfig<any> | undefined = undefined,
    E = unknown,
> =
 ...
 */
export type ValidateRepoConfig<
    T extends object,
    M extends Prisma.ModelName,
    Config,
> = {
    ...
    methods?: {
        [K in keyof (Config extends { methods: infer Meth }
            ? Meth
            : {})]: K extends string
            ? HasMultipleANDs<K> extends true
                ? MethodConfig<
                      M,
                      Config extends { selectModels: infer SM } ? SM : any
 ...
 */
export declare function setupVSRepo<
    T extends object,
    M extends Prisma.ModelName,
>(): <
